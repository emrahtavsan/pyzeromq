...
class MajorDomoWorker(object):
    ...
    def __init__(self, broker, service, verbose=False, worker_name=None):
        self.broker = broker
        ...
        logging.basicConfig(format="%(asctime)s %(message)s",
                            datefmt="%Y-%m-%d %H:%M:%S",
                            level=logging.INFO)
        self.reconnect_to_broker(self.worker_name)


    def reconnect_to_broker(self, worker_name):
        ...
        self.send_to_broker(MDP.W_READY, self.service, [], worker_name=worker_name)
    ...
    def send_to_broker(self, command, option=None, msg=None, worker_name=None):
        ...
        if option:
            msg = [option] + msg
        if (worker_name != None):
            msg = [b'', MDP.W_WORKER[""+ str(worker_name) +""]["name"], command] + msg
            if self.verbose:
    ...
    def recv(self, reply=None, worker=None):
        ...
        while True:
            ...
            if items:
                ...
                header = msg.pop(0)
                if header != "None":
                    header == MDP.W_WORKER[""+worker+""]["name"]
                else:
                    header == MDP.W_WORKER[""+worker+""]["name"]
                ...
                if command == MDP.W_REQUEST:
                    ...
                    self.reconnect_to_broker(worker)
                else :
            ...
            if time.time() > self.heartbeat_at:
                self.send_to_broker(MDP.W_HEARTBEAT, worker_name = worker)
                self.heartbeat_at = time.time() + 1e-3*self.heartbeat
