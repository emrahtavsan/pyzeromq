...
class MajorDomoBroker(object):
    ...
    def mediate(self):
        ...
        while True:
            ...
            if items:
                ...
                if (header in MDP.C_CLIENT):
                    client_thread = threading.Thread(target=self.process_client(sender, msg), args=(1,))
                    client_thread.start()
                elif (header in MDP.W_WORKER):
                    worker_thread = threading.Thread(target=self.process_worker(sender, msg), args=(1,))
                    worker_thread.start()
    ...
    def delete_worker(self, worker, disconnect):
        ...
        if disconnect:
            self.send_to_worker(worker, MDP.W_DISCONNECT, None, None, None)
    ...
    def service_internal(self, service, msg, client_val_name):
        ...
        msg = msg[:2] + [MDP.C_CLIENT[""+str(client_val_name)+""], service] + msg[2:]
        self.socket.send_multipart(msg)
    ...
    def send_heartbeats(self):
        ...
        if (time.time() > self.heartbeat_at):
            ...
            for worker in self.waiting:
                self.send_to_worker(worker, MDP.W_HEARTBEAT, None, None,None)
    ...
    def dispatch(self, service, msg):
        ...
        tmp_worker_for_logger = None 
        logger_worker_name = None
        ...
        while service.waiting and service.requests:
            ...
            for wrk in self.waiting:
                print( "Emrah worker name ==>>", wrk.service.name)
                if worker.service.name == wrk.service.name:
    ...
    def send_to_worker(self, worker, command, option, msg=None, send_worker_name=None):
        ...
        if send_worker_name != "None":

            msg = [worker.address, b'', ""+str(send_worker_name)+"", command] + msg
